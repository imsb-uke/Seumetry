#' Calculates median fluorescence intensity per group based on "data" layer in DefaultAssay(seu)
#'
...
#'
#' This function uses a specified assay and layer from a Seurat object, converts the Object to a flowFrame, and exports the flowFrame as a FCS file. This can be used, for example, to visualize specific clusters or cells in a traditional Flow Cytometry Software.
#'
...
export_fcs <- function(seu,
    ...
    ff <- convert_seurat(seu, "FF", assay = assay, layer = layer)
    # export fcs
...
#'
#' Takes Seurat object as input and returns FlowFrame, FlowSet or SingleCellExperiment Object. These can be used for integration of third-party tools. By default uses DefaultAssay and layer from Seurat object as Matrix. Only channels used in matrix and present in panel are used for conversion. Unused Channels will not be included.
#'
#' @param seu A seurat object.
...
convert_seurat <- function(seu,
    ...
    mtx <- as.matrix(GetAssayData(seu, assay = assay, layer = layer))
    # get panel data
    ...
    if(to == "FS"){
        ...
        ff_list <- sapply(names(seu_split), function(x) convert_seurat(seu_split[[x]], to = "FF", assay = assay, layer = layer))
        # generate flowset
        fs <- flowSet(ff_list)
        # return FlowSet
        return(fs)
