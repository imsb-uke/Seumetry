...
Derandomization: For CyTOF data, it might be desired to derandomize data, which
...
# Preprocessing
## Bead normalisation
For CyTOF data, bead normalisation may be required. This vignette uses a flow
cytometry test dataset, so bead normalisation is not applied. Please refer to
section "Other features" for a description of bead normalisation implementation.
...
as CCA integration or harmony can also be used.
...
plot1 <- Seurat::DimPlot(seu, label = TRUE) + set_theme
plot2 <- Seurat::DimPlot(seu, group.by = "sample_id") + set_theme
plot3 <- Seurat::DimPlot(seu, group.by = "layer") + set_theme
plot1 + plot2 + plot3
```

```{r seurat-vln, fig.width=10, fig.height=5}
Seurat::VlnPlot(seu, features = c("CD8", "CD4"), pt.size = 0) & set_theme
```
...
bead normalisation is only available for CyTOF data. Here, we implemented 
...
Now we need to prepare the raw data a bit so it can be loaded into Seumetry.
...
assay. Transformation should be done afterwards. 
```{r bead-normalisation}
seu_cat <- bead_norm(seu_cat, k = 50)
```
...
As an example, we convert the Seurat Object to a flowFrame object and use a gaussNorm 
batch correction method from [flowStats](https://github.com/RGLab/flowStats). 
